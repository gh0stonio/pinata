...
Piñata bundles zmx locally. Before opening a remote terminal, it checks the host for zmx and offers to install pinned zmx `0.7.0` in `~/.local/bin`. The installer supports macOS and Linux on arm64 and x86_64, requires `curl`, and verifies the archive checksum. Piñata supports existing SSH aliases, remote Git inspection, file browsing, worktree provisioning and cleanup, durable SSH terminal panes, and remote pull request lookup through `gh`. A repository can select one of the profiles available on that host, defaulting to the active profile. Profile discovery, token lookup, pull request metadata, and checks all run on the remote host. Connection editing, interactive SSH authentication, and port-forwarding configuration are intentionally out of scope. SSH terminals retain `LocalForward` entries already configured for their alias.

See [Pull request status](pull-request-status.md) for profile selection, caching, and query behavior.
